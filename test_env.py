...
if USE_PROXY:
    os.environ["http_proxy"] = os.getenv("HTTP_PROXY", "http://127.0.0.1:10809")
    os.environ["https_proxy"] = os.getenv("HTTPS_PROXY", "http://127.0.0.1:10809")
...
def test_llm():
    ...
    try:
        ...
        print(f"  将继续执行，并尝试使用 OpenAI 兼容 API 作为备选")
...
def test_notification():
    ...
    if channels:
        ...
        if config.wechat_webhook_url:
            wp = config.wechat_webhook_url
            webhook_preview = wp[:50] + "..." if len(wp) > 50 else wp
            print(f"    企业微信 URL: {webhook_preview}")
...
如果您收到此消息，说明通知功能配置正确 ✓"""
    ...
    try:
        ...
        elif NotificationChannel.WECHAT in channels:
            success = service.send_to_wechat(test_message)
        elif NotificationChannel.TELEGRAM in channels:
            success = service.send_to_telegram(test_message)
        else:
            print(f"  ✗ 没有可用的推送渠道")
            return False
